...
int GetEnemyAtWorldPos(const PlacedEnemy& enemy);//敵の配置位置を取得する
Rectangle GetEnemyRect(const PlacedEnemy& enemy);//敵の矩形を取得する
void EditorAddEnemy(StageEditor& ed, int enemyIdx);//敵を追加する
void EditorSetEnemyParam(StageEditor& ed, int enemyIdx, int paramIdx, float value);//敵のパラメータを設定する

void DrawnEnemyPropertyPanel(StageEditor& ed);//敵のプロパティパネルを描画する)